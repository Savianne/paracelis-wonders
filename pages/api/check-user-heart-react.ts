...
export default async function handler(
  ...
  res: NextApiResponse
    ...
    try {
        const userHeartReact = (await promisePool.query("SELECT COUNT(*) AS count FROM heartreact WHERE userEmail = ? AND destinationUID = ?", [userEmail, placeUID]) as RowDataPacket[])[0][0].count;
        
        res.status(200).json({ userHeartReact });
    } catch(err) {
        console.log(err)
