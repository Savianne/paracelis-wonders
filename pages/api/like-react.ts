  ...
  res: NextApiResponse
    ...
    try {
        ...
        try {
            ...
            await connection.query("DELETE FROM likes WHERE userEmail = ? AND destinationUID = ?", [userEmail, placeUID]);

            const affectedRows = (await connection.query("INSERT INTO likes (userEmail, destinationUID) VALUES (?, ?)", [userEmail, placeUID]) as OkPacketParams[])[0].affectedRows;
